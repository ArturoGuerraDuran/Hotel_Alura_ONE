...
public class Busqueda extends JFrame {
	...
	private List<ModeloReservaciones> buscarReserva(){
		return this.reservasController.Buscar();
	}
	private List<ModeloReservaciones> BuscarReservasId() {
		return this.reservasController.Buscarid(txtBuscar.getText());
	} 
	
	
	private void LlenarTablaReservas() {
		List<ModeloReservaciones> reserva=buscarReserva();
		try {
			for(ModeloReservaciones reservas : reserva) {
				modelo.addRow(new Object[] {reservas.getId(), reservas.getCheckIn(), reservas.getCheckOut(), reservas.getvalor(), reservas.getformaPago()});
	...
	private void LlenarTablaReservasId() {
		...
		List<ModeloReservaciones> reserva = BuscarReservasId();
		try {
			for (ModeloReservaciones reservas : reserva) {
				modelo.addRow(new Object[] { reservas.getId(), reservas.getCheckIn(), reservas.getCheckOut(), reservas.getvalor(), reservas.getformaPago() });
			}
	...
	private List<ModeloHuespedes> buscarHuesped(){
		return this.huespedesController.listarHuespedes();
	}
	private List<ModeloHuespedes> BuscarHuespedId() {
		return this.huespedesController.BuscarId(txtBuscar.getText());
	} 
	
	
	private void LlenarTablaHuespedes() {
		List<ModeloHuespedes> huesped=buscarHuesped();
		try {
			for(ModeloHuespedes huespedes : huesped) {
				modeloHuesped.addRow(new Object[] {huespedes.getId(), huespedes.getNombre(), huespedes.getApellido(), huespedes.getFecha_de_nacimiento(), huespedes.getNacionalidad(), huespedes.getTelefono(),huespedes.getId_Reserva()});
	...
	private void LlenarTablaHuespedesId() {
		...
		List<ModeloHuespedes> huesped = BuscarHuespedId();
		try {
			for (ModeloHuespedes huespedes : huesped) {
				modeloHuesped.addRow(new Object[] { huespedes.getId(), huespedes.getNombre(), huespedes.getApellido(), huespedes.getFecha_de_nacimiento(), huespedes.getNacionalidad(), huespedes.getTelefono(),huespedes.getId_Reserva()});
			}
